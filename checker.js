(function () {
  ...
  function hints(done) {
    ...
    webrtcIps(function (ips) {
      console.log('[nova-checker] WebRTC IPs:', ips);
      ips.forEach(push);
      console.log('[nova-checker] All hints:', list);
      done(list);
    });
  }
  ...
  function plan(hintList, deep) {
    ...
    if (manual !== 'auto') {
      console.log('[nova-checker] Using manual subnet:', manual);
      range(manual);
      return jobs;
    }
    ...
    console.log('[nova-checker] Detected subnets:', near);
    var known = near.length > 0;
    if (!known) near.push('192.168.1', '192.168.0');
    console.log('[nova-checker] Will scan subnets:', near);
