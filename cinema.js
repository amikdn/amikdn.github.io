;(function () {
  ...
  ;(function () {
    ...
    function initRussianPlugin() {
      ...
      function rusMovieComponent(data) {
        ...
        category.use({
            ...
            setTimeout(() => {
              const results = {
                results: categories.map(item => ({
                  title: item.title,
                  img: item.img,
                  params: { style: { name: 'collection' }, module: Lampa.Maker.module('Card').only('Card', 'Callback', 'Style') },
                  data: { url: item.request, title: item.title, component: 'category_full', source: 'tmdb', page: 1 }
                }))
              };
          ...
          onInstance: function (instance, card) {
            instance.use({
              onlyEnter: function () {
                card && card.data && Lampa.Activity.push(card.data);
              }
            });
          }
        });
      ...
      const menuItem = $(`<li class="menu__item selector"><div class="menu__ico">${iconSvg}</div><div class="menu__text">${pluginInfo.name}</div></li>`);
      menuItem.on('hover:enter', () => {
        Lampa.Activity.push({ url: '', title: pluginInfo.name, component: 'rus_movie', page: 1 });
      });
      $('.menu .menu__list').eq(0).append(menuItem);

      // Кастомная карточка для сериалов (если нужна, оставлена из оригинала)
      const episodeCard = function (data) {
        const cardData = data.card || data;
        const episode = data.next_episode_to_air || data.episode || {};
        ...
        function remove(el) { if (el) el.remove(); }
        ...
        this.build = function () {
          ...
          if (episode && episode.air_date) {
            this.card.querySelector('.full-episode__name').innerText = episode.name || 'Без названия';
        ...
        this.create = function () {
          this.build();
          this.card.addEventListener('hover:focus', () => { if (this.onFocus) this.onFocus(this.card, cardData); });
          this.card.addEventListener('hover:hover', () => { if (this.onHover) this.onHover(this.card, cardData); });
          this.card.addEventListener('hover:enter', () => { if (this.onEnter) this.onEnter(this.card, cardData); });
          this.image();
        };
        ...
        this.render = function (jq) { return jq ? this.card : $(this.card); };
      };
      ...
      class RussianMain {
        ...
        main(params = {}, onSuccess, onError) {
          ...
          const startDate1 = `${randomRange1.start}-01-01`;
          const endDate1 = `${randomRange1.end}-12-31`;
          const startDate2 = `${randomRange2.start}-01-01`;
          const endDate2 = `${randomRange2.end}-12-31`;

          const sortOptions1 = ['vote_count.desc', 'vote_average.desc', 'popularity.desc', 'revenue.desc'];
          const sortOptions2 = ['vote_count.desc', 'popularity.desc', 'revenue.desc'];

          const randomSort1 = sortOptions1[Math.floor(Math.random() * sortOptions1.length)];
          const randomSort2 = sortOptions2[Math.floor(Math.random() * sortOptions2.length)];

          const lines = [
            cb => this.original.get('movie/now_playing', params, data => {
            ...
            cb => this.original.get('trending/all/day', params, data => {
            ...
            cb => this.original.get('trending/all/week', params, data => {
            ...
            cb => this.original.get(`discover/movie?vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=primary_release_date.desc&primary_release_date.lte=${today}`, params, data => {
            ...
            cb => this.original.get(`discover/tv?with_original_language=ru&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
            ...
            cb => this.original.get('movie/upcoming', params, data => {
            ...
            cb => this.original.get(`discover/movie?vote_average.gte=5&vote_average.lte=9.5&with_genres=16&with_original_language=ru&sort_by=primary_release_date.desc&primary_release_date.lte=${today}`, params, data => {
            ...
            cb => this.original.get('movie/popular', params, data => {
            ...
            cb => this.original.get('trending/tv/week', params, data => {
            ...
            cb => this.original.get(`discover/movie?primary_release_date.gte=${startDate2}&primary_release_date.lte=${endDate2}&vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=${randomSort2}`, params, data => {
            ...
            cb => this.original.get(`discover/tv?first_air_date.gte=${startDate1}&first_air_date.lte=${endDate1}&with_networks=2493|2859|4085|3923|3871|3827|5806|806|1191&sort_by=${randomSort1}`, params, data => {
            ...
            cb => this.original.get(`discover/tv?with_networks=2493&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'Start';
            ...
            cb => this.original.get(`discover/tv?with_networks=2859&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'Premier';
            ...
            cb => this.original.get(`discover/tv?with_networks=4085&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'KION';
            ...
            cb => this.original.get(`discover/tv?with_networks=3923&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'ИВИ';
            ...
            cb => this.original.get(`discover/tv?with_networks=3871&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'Okko';
            ...
            cb => this.original.get(`discover/tv?with_networks=3827&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
            ...
            cb => this.original.get(`discover/tv?with_networks=5806&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'Wink';
            ...
            cb => this.original.get(`discover/tv?with_networks=806&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'СТС';
            ...
            cb => this.original.get(`discover/tv?with_networks=1191&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => {
              data.title = 'ТНТ';
            ...
            cb => this.original.get('movie/top_rated', params, data => {
            ...
            cb => this.original.get('tv/top_rated', params, data => {
          ...
          Lampa.ContentRows.call('main', params, lines);

          const totalLines = lines.length + 1;
          Lampa.Arrays.insert(lines, 0, Lampa.Api.partPersons(lines, 6, 'movie', totalLines));

          this.original.genres.movie.forEach(genre => {
            lines.push(cb => this.original.get(`discover/movie?with_genres=${genre.id}`, params, data => {
              data.title = Lampa.Lang.translate(genre.title.replace(/[^a-z_]/g, ''));
              cb(data);
            }, cb));
          });

          function partNext(start, limit) {
            Lampa.Api.partNext(lines, 6, start, limit);
          }
      ...
      // Все SVG-иконки (полностью из оригинала)
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: {
          ...
          description: 'Показывать подборки русских новинок на главной странице. После изменения перезапустите приложение (работает только с TMDB)'
        },
        onRender: element => {
          setTimeout(() => {
