(function () {
    ...
    // === Добавление фильтра (ОФИЦИАЛЬНЫЙ СПОСОБ) ===
    function addWebDLFilter() {
        if (filterAdded || !Lampa.Filter) return;
        filterAdded = true;
        ...
        if (Lampa.Filter.update) {
            setTimeout(() => Lampa.Filter.update(), 100);
        }
    }
    ...
    // === Запуск ===
    function start() {
        if (window.appready && Lampa.Filter) {
            addWebDLFilter();
            setupUrlChange();
            applyFilterOnLoad();
