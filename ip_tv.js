...
function networkSilentSessCache(url, success, fail, param) {
    ...
    if (data) {
        data = JSON.parse(data);
        if (data[0]) {
            console.log(plugin.name, 'Loaded from cache:', url);
            typeof success === 'function' && success.apply(context, [data[1]]);
        } else {
            logError('Cached error for URL: ' + url + ', Error: ' + JSON.stringify(data[1]));
            sessionStorage.removeItem(key); // Очистка кэша при ошибке
            typeof fail === 'function' && fail.apply(context, [data[1]]);
        }
    } else {
        var network = new Lampa.Reguest();
        network.silent(
            url,
            function (data) {
                console.log(plugin.name, 'Successfully fetched:', url);
                sessionStorage.setItem(key, JSON.stringify([true, data]));
                typeof success === 'function' && success.apply(context, [data]);
            },
            function (data) {
                logError('Network request failed for URL: ' + url + ', Error: ' + JSON.stringify(data));
...
function pluginPage(object) {
    var html = $('<div></div>'),
        body = $('<div class="category-full ' + plugin.component + '"></div>'),
        scroll = new Lampa.Scrol|| '').toLowerCase();
	source = source || '';
	if (!type) {
		if (!!source) {
			if (source.search(/^https?:\/\//i) === 0) type = 'default';
...
function networkSilentSessCache(url, success, fail, param) {
	...
	if (data) {
		...
		network.silent(
			...
			function (data) {
				console.error(plugin.name, 'Network request failed for URL:', url, 'Error:', data);
				sessionStorage.setItem(key, JSON.stringify([false, data]));
				typeof fail === 'function' && fail.apply(context, [data]);
			},
			param
...
function pluginPage(object) {
	var html = $('<div></div>'),
		...
		info, last, favorite, layerCards;
	function setEpgId(catalog) {
		for (var k in catalog.channels) {
			var tvgId = catalog.channels[k]['tvg-id'] || '';
			if (!tvgId) {
				var title = favID(catalog.channels[k].title);
				catalog.channels[k]['tvg-id'] = title;
			}
			if (!catalog.channels[k]['tvg-logo']) {
				if (catalog.channels[k].title.search(/^Ch \d+$/) === 0) {
					catalog.channels[k]['tvg-logo'] = Lampa.Utils.protocol() + 'epg.rootu.top/picon/' + catalog.channels[k].title + '.png'
				} else if (isSNG && !/^Ch \d+$/.test(catalog.channels[k]['Title'])) {
					catalog.channels[k]['tvg-logo'] = Lampa.Utils.protocol() + 'epg.rootu.top/picon/'
						+ encodeURIComponent(catalog.channels[k]['Title']) + '.png';
				}
			}
		}
	}
	this.append = function (items) {
		...
		items.forEach(function (element) {
			var card = $('<div class="card card--collection selector"><div class="card__view"><div class="card__img"></div></div><div class="card__info"><div class="card__title">' + element.title + '</div></div></div>');
			var img = card.find('.card__img');
			if (element['tvg-logo']) {
				img.append('<img class="card__img" src="' + element['tvg-logo'] + '" onerror="this.onerror=null; this.src=\'' + Lampa.Utils.protocol() + 'epg.rootu.top/picon/' + encodeURIComponent(element.title) + '.png\'" />');
			} else {
				img.text(element.title.substring(0, 2));
			}
			card.on('hover:focus', function (e) {
				last = card[0];
				Lampa.Controller.collectionFocus(card[0], scroll.render());
			}).on('hover:enter', function (e) {
				var item = Lampa.Arrays.clone(element);
				item.tv = true;
				item.plugin = plugin.component;
				Lampa.Player.play(item);
				Lampa.PlayerPlaylist.add([item]);
				Lampa.PlayerPlaylist.listener.send('select', {item: item, playlist: [item], position: 0});
				Lampa.Background.immediately(Lampa.Utils.cardImgBackgroundBlur(item));
			});
			body.append(card);
		});
	...
	this.build = function () {
		var style = Lampa.Storage.get(plugin.component + '_square_icons', 'false') === 'true' ? ' square_icons' : '';
		body.addClass('category-full' + style);
		if (Lampa.Storage.get(plugin.component + '_contain_icons', 'true') === 'true') body.addClass('contain_icons');
		this.activity.loader(true);
		var maxCh = parseInt(getStorage('max_ch_in_group', '300'));
...
langAdd('default_playlist',
	{
		ru: 'https://raw.githubusercontent.com/IPTVSHARED/iptv/refs/heads/main/IPTV_SHARED.m3u',
		uk: 'https://raw.githubusercontent.com/IPTVSHARED/iptv/refs/heads/main/IPTV_SHARED.m3u',
		be: 'https://raw.githubusercontent.com/IPTVSHARED/iptv/refs/heads/main/IPTV_SHARED.m3u',
		en: 'https://raw.githubusercontent.com/IPTVSHARED/iptv/refs/heads/main/IPTV_SHARED.m3u',
		zh: 'https://raw.githubusercontent.com/IPTVSHARED/iptv/refs/heads/main/IPTV_SHARED.m3u'
	}
);
...
langAdd('settings_list_url_desc0',
	{
		ru: 'По умолчанию используется плейлист из проекта <i>https://github.com/IPTVSHARED/iptv</i><br>Вы можете заменить его на свой.',
		uk: 'За замовчуванням використовується плейлист із проекту <i>https://github.com/IPTVSHARED/iptv</i><br>Ви можете замінити його на свій.',
		be: 'Па змаўчанні выкарыстоўваецца плэйліст з праекта <i>https://github.com/IPTVSHARED/iptv</i><br> Вы можаце замяніць яго на свой.',
		en: 'The default playlist is from the project <i>https://github.com/IPTVSHARED/iptv</i><br>You can replace it with your own.',
		zh: '默认播放列表来自项目 <i>https://github.com/IPTVSHARED/iptv</i><br>您可以将其替换为您自己的。'
	}
);
