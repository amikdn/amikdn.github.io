...
Lampa.PlayerPlaylist.listener.follow('select', function(e) {
    ...
    Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
});
...
Lampa.Player.runas && Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
    }
...
function toLocaleTimeString(time) {
    var date = new Date(),
ofst = parseInt([Lampa.Storage](http://Lampa.Storage).get('time_offset', 'n0').replace('n',''));
...
var utils = {
    ...
    timestamp: unixtime,
    token: function() {return generateSigForString([Lampa.Storage](http://Lampa.Storage).field('account_email').toLowerCase())},
...
function tf(t, format, u, tz) {
    ...
    var thisOffset = 0;
    thisOffset += tz * 60;
    if (!u) thisOffset += parseInt([Lampa.Storage](http://Lampa.Storage).get('time_offset', 'n0').replace('n','')) * 60 - new Date().getTimezoneOffset();
...
function catchupUrl(url, type, source) {
    ...
    if (!type) {
if (!!source) {
    if ([source.search](http://source.search)(/^https?:\/\//i) === 0) type = 'default';
    else if ([source.search](http://source.search)(/^[?&/][^/]/) === 0) type = 'append';
    else type = 'default';
}
else if (url.indexOf('${') < 0) type = 'shift';
else type = 'default';
console.log([plugin.name](http://plugin.name), 'Autodetect catchup-type "' + type + '"');
    }
    var newUrl = '';
    switch (type) {
case 'append':
    if (source) {
newUrl = ([source.search](http://source.search)(/^https?:\/\//i) === 0 ? '' : url) + source;
...
case 'disabled':
    return false;
default:
    console.log([plugin.name](http://plugin.name), 'Err: no support catchup-type="' + type + '"');
    return false;
    }
...
if (code === 428 || code === 34 // Pg-
    //4 - Samsung orsay
    || ((code === 37 || code === 4) && !$('.[player.tv](http://player.tv) .panel--visible .focus').length) // left
) {
    ...
    || ((code === 39 || code === 5) && !$('.[player.tv](http://player.tv) .panel--visible .focus').length) // right
) {
...
if (Object.keys(catalog).length) {
    _[this.build](http://this.build)(
!catalog[object.currentGroup]
    ? (lists[[object.id](http://object.id)].groups.length > 1 && catalog[lists[[object.id](http://object.id)].groups[1].key]
? catalog[lists[[object.id](http://object.id)].groups[1].key]['channels']
: []
    )
    : catalog[object.currentGroup]['channels']
    );
} else if(!lists[[object.id](http://object.id)] || !object.url) {
    emptyResult();
    return;
} else {
...
lists[[object.id](http://object.id)].groups = [{
    title: langGet('favorites'),
    key: ''
}];
var l = data.split(/\r?\n/);
...
// else if (!!(m = l[i].match(/^(https?|udp|rt[ms]?p|mms|acestream):\/\/(.+)$/i))) {
    ...
    [channel.Group](http://channel.Group) = channel['group-title'] || defGroup;
    cnt++;
}
    }
    if (!!channel.Url && !channel.isYouTube) {
if (!catalog[[channel.Group](http://channel.Group)]) {
    catalog[[channel.Group](http://channel.Group)] = {
title: [channel.Group](http://channel.Group),
channels: []
    };
    lists[[object.id](http://object.id)].groups.push({
title: [channel.Group](http://channel.Group),
key: [channel.Group](http://channel.Group)
    });
}
...
catalog[[channel.Group](http://channel.Group)].channels.push(channel);
...
for (i = 0; i < lists[[object.id](http://object.id)].groups.length; i++) {
    var group = lists[[object.id](http://object.id)].groups[i];
...
cIdEl = [epgEl.data](http://epgEl.data)('cId') || '';
if (cIdEl !== cId) {
    [epgEl.data](http://epgEl.data)('cId', cId);
    [epgEl.data](http://epgEl.data)('progress', p);
    epgEl.find('.js-epgTitle').text(e[2]);
    epgEl.find('.js-epgProgress').css('width', p + '%');
    [epgEl.show](http://epgEl.show)();
} else if ([epgEl.data](http://epgEl.data)('progress') !== p) {
    [epgEl.data](http://epgEl.data)('progress', p);
    epgEl.find('.js-epgProgress').css('width', p + '%');
}
    }
...
list.append(item);
    ...
    [epgAfter.show](http://epgAfter.show)();
} else {
    epgAfter.hide();
}
    } else if (i && [epgNow.data](http://epgNow.data)('progress') !== p) {
[epgNow.data](http://epgNow.data)('progress', p);
epgNow.find('.js-epgProgress').css('width', p + '%');
    }
}
...
last = card[0];
console.log([plugin.name](http://plugin.name), event.type);
...
Lampa.Player.runas && Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
[Lampa.Player.play](http://Lampa.Player.play)(video);
Lampa.Player.runas && Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
...
[Lampa.Select.show](http://Lampa.Select.show)({
...
[Lampa.Select.show](http://Lampa.Select.show)({
    ...
    items: archiveMenu,
    onSelect: function (sel) {
console.log([plugin.name](http://plugin.name), 'catchupUrl: ' + sel.catchupUrl, epg.slice(0,2));
var video = {
    title: sel.title,
    url: sel.url,
    playlist: playlist
...
(channel['catchup-source'] || listCfg['catchup-source'])
    ...
    var flussonic = [type.search](http://type.search)(/^flussonic/i) === 0;
    if (flussonic) {
url = url.replace('${(d)S}', 'now');
    }
    console.log([plugin.name](http://plugin.name), 'catchupUrl: ' + url, epg.slice(0,2));
    var video = {
title: channel.Title,
url: prepareUrl(url, epg),
plugin: plugin.component,
...
epg: epg
    ...
    Lampa.Controller.toggle('content');
    Lampa.Player.runas && Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
    [Lampa.Player.play](http://Lampa.Player.play)(video);//[Lampa.PlayerVideo.to](http://Lampa.PlayerVideo.to)(0)
    Lampa.Player.runas && Lampa.Player.runas([Lampa.Storage](http://Lampa.Storage).field('player_iptv'));
...
$('#' + plugin.component + '_epg').remove();
    ...
    var favGroup = lists[[object.id](http://object.id)].groups[0];
    if (!!sel.favToggle) {
if (favI === -1) {
...
catalog[favGroup.key].channels.splice(sel.i, 0, channel);
    ...
    setStorage('favorite' + [object.id](http://object.id), favorite);
    favGroup.title = catalog[favGroup.key].title
+ ' [' + catalog[favGroup.key].channels.length + ']';
    if (isFavoriteGroup) {
Lampa.Activity.replace(Lampa.Arrays.clone(lists[[object.id](http://object.id)].activity));
    } else {
...
scroll.render().parent().append(epgTemplate);
    ...
    scroll.append(body);
    setStorage('last_catalog' + [object.id](http://object.id), object.currentGroup ? object.currentGroup : '!!');
    lists[[object.id](http://object.id)].activity.currentGroup = object.currentGroup;
} else {
    var empty = new Lampa.Empty();
    html.append(empty.render());
...
var activity = Lampa.Arrays.clone(lists[[object.id](http://object.id)].activity);
[Lampa.Select.show](http://Lampa.Select.show)({
    title: langGet('categories'),
    items: Lampa.Arrays.clone(lists[[object.id](http://object.id)].groups),
...
function getStorage(name, defaultValue) {
    return [Lampa.Storage](http://Lampa.Storage).get(plugin.component + '_' + name, defaultValue);
}
function setStorage(name, val, noListen) {
    return [Lampa.Storage](http://Lampa.Storage).set(plugin.component + '_' + name, val, noListen);
}
function getSettings(name) {
    return [Lampa.Storage](http://Lampa.Storage).field(plugin.component + '_' + name);
}
function addSettings(type, param) {
    var data = {
component: plugin.component,
param: {
    name: plugin.component + '_' + [param.name](http://param.name),
    type: type,
...
field: {
    name: !param.title ? (![param.name](http://param.name) ? '' : [param.name](http://param.name)) : param.title
}
    }
    if (!![param.name](http://param.name)) [data.param.name](http://data.param.name) = plugin.component + '_' + [param.name](http://param.name);
...
var activity = {
    id: 0,
    url: '<http://4m1k.github.io/tv.m3u>',
    title: [plugin.name](http://plugin.name),
    groups: [],
...
var menuEl = $('<li class="menu__item selector js-' + plugin.component + '-menu0">'
    ...
    + '<div class="menu__text js-' + plugin.component + '-menu0-title">'
        + encoder.text([plugin.name](http://plugin.name)).html()
    + '</div>'
+ '</li>')
.on('hover:enter', function(){
