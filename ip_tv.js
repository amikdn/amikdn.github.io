...
    Title: "#" + favorite[i],
    isYouTube: false,
    Url: Lampa.Utils.protocol() + 'epg.rootu.top/empty/*.m3u8',
    Group: '',
    Options: {},
    'tvg-logo': Lampa.Utils.protocol() + 'epg.rootu.top/empty/*.gif'
};
    }
}
*this.build(
...
    var ec = $('#' + plugin.component + '*epg');
    var epgNow = ec.find('.js-epgNow');
    cId = epgId + '*' + epg.length + (epg.length ? '_' + epg[0][0] : '');
    cIdEl = ec.data('cId') || '';
    if (cIdEl !== cId) {
ec.data('cId', cId);
...
Lampa.Template.add(plugin.component + '*button_category', "<style>@media screen and (max-width: 2560px) {." + plugin.component + " .card--collection {width: 16.6%!important;}}@media screen and (max-width: 800px) {." + plugin.component + " .card--collection {width: 24.6%!important;}}@media screen and (max-width: 500px) {." + plugin.component + " .card--collection {width: 33.3%!important;}}</style><div class="full-start__button selector view--category"><svg style="enable-background:new 0 0 512 512;" version="1.1" viewBox="0 0 24 24" xml:space="preserve" xmlns="<http://www.w3.org/2000/svg\>" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="info"/><g id="icons"><g id="menu"><path d="M20,10H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h16c1.1,0,2-0.9,2-2C22,10.9,21.1,10,20,10z" fill="currentColor"/><path d="M4,8h12c1.1,0,2-0.9,2-2c0-1.1-0.9-2-2-2H4C2.9,4,2,4.9,2,6C2,7.1,2.9,8,4,8z" fill="currentColor"/><path d="M16,16H4c-1.1,0-2,0.9-2,2c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2C18,16.9,17.1,16,16,16z" fill="currentColor"/></g></g></svg><span>" + langGet('categories') + "</span>\n </div>");
Lampa.Template.add(plugin.component + '*info_radio', '<div class="info layer--width"><div class="info__left"><div class="info__title"></div><div class="info__title-original"></div><div class="info__create"></div></div><div class="info__right" style="display: flex !important;"> <div id="stantion_filtr"></div></div></div>');
var btn = Lampa.Template.get(plugin.component + '*button_category');
info = Lampa.Template.get(plugin.component + '*info_radio');
...
    langData[plugin.component + '*' + name] = values;
}
function langGet(name) {
    return Lampa.Lang.translate(plugin.component + '*' + name);
}
langAdd('categories',
    {
ru: 'Категории'
...
    return Lampa.Storage.get(plugin.component + '*' + name, defaultValue);
}
function setStorage(name, val, noListen) {
    return Lampa.Storage.set(plugin.component + '*' + name, val, noListen);
}
function getSettings(name) {
    return Lampa.Storage.field(plugin.component + '*' + name);
}
function addSettings(type, param) {
    var data = {
component: plugin.component,
param: {
    name: plugin.component + '*' + param.name,
    type: type,
...
    }
    if (!) data.param.name = plugin.component + '*' + param.name;
...
        + encoder.text<a href="http://plugin.name" target="_blank" rel="noopener noreferrer nofollow"></a>.html()
    + '</div>'
'</li>')
.on('hover:enter', function(){
...
    if (!!window['plugin_' + plugin.component + '*ready']) return;
    window['plugin*' + plugin.component + '_ready'] = true;
