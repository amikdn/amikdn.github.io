...
langAdd('playlist_url_desc', {
    ru: 'Введите URL вашего IPTV плейлиста (например, http://example.com/playlist.m3u)'
});
...
addSettings(
    ...
    {
        name: 'settings_title',
        title: langGet('settings_title') // Отображается как "Hack TV PlayList"
    }
);

// Регистрация пункта меню в левом боковом меню
if (typeof Lampa.MainMenu !== 'undefined' && typeof Lampa.MainMenu.add === 'function') {
    Lampa.MainMenu.add({
        title: plugin.name, // Название в боковом меню: "Hack TV"
        ...
        action: function () {
            ...
            Lampa.Activity.push({
                component: plugin.component,
                id: 0,
                url: getSettings('playlist_url') || '', // Используем URL из настроек
                currentGroup: getStorage('last_catalog0', '') || '' // Восстанавливаем последнюю группу
            });
        }
    });
} else {
    console.warn('Lampa.MainMenu.add is not available, falling back to alternative method');
    // Альтернативный способ добавления пункта меню
    Lampa.Component.add(plugin.component, pluginPage);
    Lampa.Listener.follow('app', function(e) {
        if (e.type === 'ready') {
            console.log('App ready, registering custom menu item');
            // Попытка добавить пункт меню через другой метод, если доступен
            if (typeof Lampa.Menu !== 'undefined' && typeof Lampa.Menu.register === 'function') {
                Lampa.Menu.register({
                    ...
                    component: plugin.component,
                    action: function () {
                        console.log('Opening IPTV with URL:', getSettings('playlist_url'));
                        Lampa.Activity.push({
                            component: plugin.component,
                            id: 0,
                            url: getSettings('playlist_url') || '',
