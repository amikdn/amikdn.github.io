...
function changeServer() {
    Lampa.Input.edit({
        ...
        if (server) window.location.href = protocol + server;
        else buildMenu();
    });
}
...
/* -------------------------------------------------------------------------- */
/*  Добавление параметров настроек (только после готовности ядра)          */
/* -------------------------------------------------------------------------- */
function registerSettings() {
    const add = (obj) => {
        // Lampa.Settings.addParam может отсутствовать → fallback
        if (typeof Lampa.Settings.addParam === 'function') {
            Lampa.Settings.addParam(obj);
        } else if (Lampa.Settings && Lampa.Settings.main) {
            Lampa.Settings.main(obj);
        }
    };
    ...
    // ----- Обычные пункты -----
    add({ component: 'back_menu', param: { name: 'exit',          type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '2' }, field: { name: 'Выход',          description: 'Нажмите для выбора' } });
    addApi({ component: 'back_menu', param: { name: 'reboot',        type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '2' }, field: { name: 'Перезагрузить', description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'clear_cache',   type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '2' }, field: { name: 'Очистить кэш',   description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'switch_server', type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '2' }, field: { name: 'Сменить сервер', description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'youtube',       type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'YouTube',        description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'rutube',        type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'RuTube',         description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'back_plug',     type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'Закрыть приложение', description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'twitch',        type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'Twitch',         description: 'Нажмите для выбора' } });
    addApi({ component: 'back_menu', param: { name: 'drm_play',      type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'DRM Play',      description: 'Нажмите для выбора' } });
    add({ component: 'back_menu', param: { name: 'fork_player',   type: 'select', values: { 1: 'Скрыть', 2: 'Отобразить' }, default: '1' }, field: { name: 'ForkPlayer',    description: 'Нажмите для выбора' } });
}
...
/* -------------------------------------------------------------------------- */
/*  Инициализация плагина (только после полной готовности)                */
/* -------------------------------------------------------------------------- */
function initBackMenu() {
    // Плейсхолдеры
    Lampa.Listener.follow('main', () => {});

    // Показать компонент back_menu при выборе в контроллере
    ...
    // Регистрация параметров настроек
    registerSettings();

    // Применить дефолты, если ещё не применялись
    if (!Lampa.Storage.get('lampa', 'false')) {
        applyDefaults();
    }
}

/* -------------------------------------------------------------------------- */
/*  Запуск – ждём полной готовности ядра Lampa                            */
/* -------------------------------------------------------------------------- */
if (typeof Lampa !== 'undefined' && Lampa.appready) {
    initBackMenu();
} else {
    // Ждём события appready
    Lampa.Listener.follow('app', (e) => {
        if (e.type === 'ready') {
            // Небольшая задержка, чтобы ядро успело создать Settings.addParam
            setTimeout(initBackMenu, 50);
        }
    });
}
