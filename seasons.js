(function () {
    ...
    var CONFIG = {
        ...
        enabled: Lampa.Storage.get('season_badge_enabled', true),
        language: 'uk'
    };
    ...
    function initSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Отображение информации о сезонах',
                description: 'Включить/отключить отображение информации о сезонах на карточках сериалов'
            },
            onChange: function (v) {
                Lampa.Storage.set('season_badge_enabled', v);
                CONFIG.enabled = v;
                Lampa.Settings.update();
                if (!v) {
                    document.querySelectorAll('.card--season-complete, .card--season-progress, .card--new_seria').forEach(el => el.remove());
                } else {
