(function () {
    ...
    function runPlugin() {
        ...
        function applyServer(id) {
            const server = servers.find(s => s.id === id) || servers[2]; // дефолт jacred_xyz
        ...
        Lampa.SettingsApi.addParam({
            ...
            onRender: element => {
                ...
                const typeBlock = $('div[data-name="parser_torrent_type"]');
                if (typeBlock.length) {
                    element.insertAfter(typeBlock);
                }
                ...
                setTimeout(() => {
                    servers.forEach((server, i) => {
                        setTimeout(() => {
                            const proto = server.url.includes('jr.maxvol.pro') ? 'https://' : location.protocol === 'https:' ? 'https://' : 'http://';
                            ...
                            xhr.onload = () => {
                                ...
                                const selector = `body > div.selectbox > div.selectbox__content > div.selectbox__body > div > div > div > div:nth-child(${i + 2}) > div`;
