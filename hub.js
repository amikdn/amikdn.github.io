(function () {
    ...
    // Проверка доступности серверов для выпадающего списка
        ...
        setTimeout(() => {
            ...
            xhr.onload = () => {
                if (xhr.status === 200) {
                    ...
                    if ($(selector).text() === serverTitles[index]) {
                        $(selector).html('✘&nbsp;&nbsp;' + serverTitles[index]).css('color', '#ffffff');
                    }
                } else {
                    if ($(selector).text() === serverTitles[index]) {
                        $(selector).html('✘&nbsp;&nbsp;' + serverTitles[index]).css('color', '#ff2121');
    ...
    function applyServerSettings() {
        ...
        if (Lampa.Storage.get('jackett_urltwo') === 'jacred_pro') {
            Lampa.Storage.set('jackett_url', 'ru.jacred.pro');
            Lampa.Storage.set('jackett_key', '');
            Lampa.Storage.set('jackett_interview', 'all');
            Lampa.Storage.set('parse_in_search', true);
            Lampa.Storage.set('parse_lang', 'lg');
        ...
        onRender: (element) => {
            setTimeout(() => {
                ...
                if (Lampa.Storage.get('parser') && Lampa.Storage.get('torrents') === 'jackett') {
                    ...
                    $('div[data-children="parser"]').insertAfter(jackettIconHtml);
                } else {
                    element.hide();
                }
            }, 5);
    ...
    Lampa.Settings.listen('timeout', (event) => {
        if (event.name === 'settings_component') {
            event.activity.find('[data-name="jackett_url2"]').hide();
            event.activity.find('div[data-name="parser_torrent_type"]').hide();
        }
    });
    ...
    Lampa.Storage.follow('parser', () => {
        if (Lampa.Storage.get('torrents') !== 'jackett') {
            ...
            $('div[data-name="jackett_urltwo"]').insertAfter(jackettIconHtml);
        }
    });
    ...
    function openParserModal() {
        ...
        Promise.all(list.map((item) => checkServerForModal(item.url, item.key, item.title, item))).then((items) => {
            Lampa.Modal.open({
                title: 'Выбрать парсер',
                items: items.map((i) => ({title: i.title, url: i.url, url_two: i.url_two, key: i.key, int: i.int, lang: i.lang})),
    ...
    Lampa.Storage.follow('parser', (event) => {
        if (event.name === 'settings_component' && Lampa.Activity.active().name === 'settings') {
            openParserModal();
        }
    });
    ...
    Lampa.Storage.follow('parser', (event) => {
        if (event.name === 'settings_component' && $('.empty__title').length && Lampa.Storage.field('torrents') === 'jackett') {
            openParserModal();
