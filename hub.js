(function () {
    ...
    // Проверка доступности серверов в выпадающем списке
    function checkServerAvailability(index) {
        setTimeout(() => {
            ...
            xhr.ontimeout = xhr.onerror = () => {
                if ($(selector).text() === serverTitles[index]) {
                    $(selector).html('✘&nbsp;&nbsp;' + serverTitles[index]).css('color', '#ff2121');
                }
            };
            ...
            xhr.onload = () => {
                if (xhr.status === 200) {
                    if ($(selector).text() === serverTitles[index]) {
                        $(selector).html('✔&nbsp;&nbsp;' + serverTitles[index]).css('color', '#64e364');
                    }
                } else {
                    if ($(selector).text() === serverTitles[index]) {
                        $(selector).html('✘&nbsp;&nbsp;' + serverTitles[index]).css('color', '#ff2121');
    ...
    function checkAllServersAvailability() {
        for (let i = 0; i < servers.length; i++) {
            checkServerAvailability(i);
        }
    }

    // Подписка на открытие настроек парсера
    Lampa.Settings.listen('settings_component', (event) => {
        if (event.name === 'parser') {
            setTimeout(checkAllServersAvailability, 10);
        }
    });

    // Применение настроек при выборе сервера
    function applySelectedServer() {
        const selected = Lampa.Storage.get('jackett_urltwo') || Lampa.Storage.get('parser_torrent_type');
        ...
        const config = settingsMap[selected] || settingsMap.jacred_xyz;
    ...
    Lampa.SettingsApi.addParam({
        ...
        onRender: (element) => {
            setTimeout(() => {
                $('div[data-name="jackett_urltwo"]').on('hover:enter', () => Lampa.Settings.update());
    ...
    Lampa.Settings.listen('timeout', (e) => {
    ...
    // Показ/скрытие раздела парсера
    Lampa.Storage.follow('parser', () => {
    ...
    // Модальное меню выбора парсера с проверкой доступности
    function checkServerForModal(domain, key, title) {
        return new Promise(resolve => {
            ...
            xhr.onload = () => {
                ...
                resolve({title: `<span style="color:${color}">${status}&nbsp;&nbsp;${title}</span>`, domain, key});
            };
            xhr.onerror = xhr.ontimeout = () => {
                resolve({title: `<span style="color:#ff2121">✘&nbsp;&nbsp;${title}</span>`, domain, key});
            };
            xhr.send();
        });
    }
    ...
    function openModalMenu() {
        const current = Lampa.Activity.active().name;

        const list = [
            {title: 'Jacred Lampa32 RU', domain: '62.60.149.237:8443', key: '', url_two: 'jac_lampa32_ru', int: 'all', lang: 'lg'},
            {title: 'Lampa Jackett', domain: 'bylampa.cc', key: '5178915kmzeLa', url_two: 'bylampa_jackett', int: 'all', lang: 'df'},
            {title: 'Jacred.xyz', domain: 'jacred.xyz', key: '', url_two: 'jacred_xyz', int: 'healthy', lang: 'lg'},
            {title: 'Jacred Maxvol Pro', domain: 'jr.maxvol.pro', key: '', url_two: 'jr_maxvol_pro', int: 'healthy', lang: 'lg'},
            {title: 'Jacred RU', domain: 'jac-red.ru', key: '', url_two: 'jacred_ru', int: 'all', lang: 'lg'},
            {title: 'Jacred Viewbox Dev', domain: 'jacred.viewbox.dev', key: '777', url_two: 'jacred_viewbox_dev', int: 'all', lang: 'lg'},
            {title: 'Jacred Pro', domain: 'ru.jacred.pro', key: '', url_two: 'jacred_pro', int: 'all', lang: 'lg'},
            {title: 'Jac Black', domain: 'jacblack.ru:9117', key: '', url_two: 'jac_black', int: 'all', lang: 'lg'}
        ];

        Promise.all(list.map(item => checkServerForModal(item.domain, item.key, item.title))).then(results => {
            Lampa.Modal.open({
                ...
                items: results.map((r, i) => ({
                    ...
                    key: list[i].key,
                    url_two: list[i].url_two,
                    int: list[i].int,
                    lang: list[i].lang
                })),
                onSelect: (item) => {
                    Lampa.Storage.set('jackett_url', item.domain);
                ...
                onBack: () => Lampa.Controller.toggle(current)
            });
        });
    }

    // Открытие модального меню по нажатию на пункт
    $(document).on('hover:enter', 'div[data-name="jackett_urltwo"]', openModalMenu);

    // Наблюдатель за открытием настроек
    let modalObserver;
