(function () {
  ...
  function initPlugin() {
    ...
    // Настройка вида (по умолчанию — сетка)
    Lampa.SettingsApi.addParam({
      component: 'interface',
      param: { name: 'rus_movie_view', type: 'select', values: { grid: 'Сетка', list: 'Список' }, default: 'grid' },
      field: { name: 'Русское: Интерфейс', description: 'Сетка — страница с обложками. Список — окно со списком.' },
    ...
    function rusMovieGridComponent(object) {
      ...
      category.use({
        ...
        onInstance: function (instance, cardData) {
          instance.use({ onlyEnter: () => cardData?.data && Lampa.Activity.push(cardData.data) });
        }
      });
      return category;
    }
    ...
    menuItem.on('hover:enter', () => {
      const view = Lampa.Storage.get('rus_movie_view', 'grid'); // исправлено: по умолчанию grid
      if (view === 'grid') {
        Lampa.Activity.push({ url: '', title: 'Русское', component: 'rus_movie_grid', page: 1 });
      } else {
        ...
        Lampa.Select.show({
          ...
          onSelect: selected => {
            ...
            if (index >= 0) Lampa.Activity.push({ url: collections[index].request, title: collections[index].title, component: 'category_full', source: 'tmdb', page: 1 });
          },
    ...
    // Настройка «Русское на главной»
    Lampa.SettingsApi.addParam({
      component: 'interface',
      param: { name: 'rus_movie_main', type: 'trigger', default: true },
      field: { name: 'Русские новинки на главной', description: 'Показывать подборки русских новинок на главной странице TMDB. Перезапустите приложение после изменения.' },
      onRender: item => setTimeout(() => $('div[data-name="rus_movie_main"]').insertAfter('div[data-name="rus_movie_view"]'), 10)
    });

    // Логика добавления русских подборок на главную TMDB
    if (Lampa.Storage.get('rus_movie_main', true)) {
      const network = new Lampa.Reguest();
      const get = (path, params, success, error) => {
        const url = Lampa.Utils.protocol() + 'api.themoviedb.org/3/' + path + (path.includes('?') ? '&' : '?') + 'page=' + (params.page || 1);
      ...
      Lampa.Api.sources.tmdb.main = function (params = {}, onSuccess, onError) {
        ...
        // Стандартные линии (как во втором плагине)
        u.push(callback => get('movie/now_playing', params, data => { data.title = Lampa.Lang.translate('title_now_watch'); callback(data); }, callback));
        u.push(callback => callback({ source: 'tmdb', results: Lampa.TimeTable.lately().slice(0, 20), title: Lampa.Lang.translate('title_upcoming_episodes'), nomore: true }));
        u.push(callback => get('trending/all/day', params, data => { data.title = Lampa.Lang.translate('title_trend_day'); callback(data); }, callback));
        u.push(callback => get('trending/all/week', params, data => { data.title = Lampa.Lang.translate('title_trend_week'); callback(data); }, callback));

        // Русские фильмы
        u.push(callback => get(collections[0].request, params, data => { data.title = 'Русские фильмы'; data.small = true; data.wide = true; data.results.forEach(r => { r.promo = r.overview; r.promo_title = r.title || r.name; }); callback(data); }, callback));
        // Русские сериалы
        u.push(callback => get(collections[1].request, params, data => { data.title = 'Русские сериалы'; callback(data); }, callback));
        u.push(callback => get('movie/upcoming', params, data => { data.title = Lampa.Lang.translate('title_upcoming'); callback(data); }, callback));
        // Русские мультфильмы
        u.push(callback => get(collections[2].request, params, data => { data.title = 'Русские мультфильмы'; data.collection = true; data.line_type = 'collection'; callback(data); }, callback));
        u.push(callback => get('movie/popular', params, data => { data.title = Lampa.Lang.translate('title_popular_movie'); callback(data); }, callback));
        u.push(callback => get('trending/tv/week', params, data => { data.title = Lampa.Lang.translate('title_popular_tv'); callback(data); }, callback));

        // Случайные подборки по годам (фильмы)
        const yearGroups = [{start:2023,end:2025},{start:2020,end:2022},{start:2017,end:2019},{start:2014,end:2016},{start:2011,end:2013}];
        const randomGroup = yearGroups[Math.floor(Math.random() * yearGroups.length)];
        const sortOptions = ['vote_count.desc','popularity.desc','revenue.desc'];
        u.push(callback => get(`discover/movie?primary_release_date.gte=${randomGroup.start}-01-01&primary_release_date.lte=${randomGroup.end}-12-31&vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=${sortOptions[Math.floor(Math.random()*sortOptions.length)]}`, params, data => { data.title = 'Подборки русских фильмов'; data.line_type = 'top'; callback(data); }, callback));

        // Случайные подборки по годам (сериалы)
        const randomGroup2 = yearGroups[Math.floor(Math.random() * yearGroups.length)];
        const sortOptions2 = ['vote_count.desc','vote_average.desc','popularity.desc','revenue.desc'];
        const networks = '2493|2859|4085|3923|3871|3827|5806|806|1191';
        u.push(callback => get(`discover/tv?first_air_date.gte=${randomGroup2.start}-01-01&first_air_date.lte=${randomGroup2.end}-12-31&with_networks=${networks}&sort_by=${sortOptions2[Math.floor(Math.random()*sortOptions2.length)]}`, params, data => { data.title = 'Подборки русских сериалов'; data.line_type = 'top'; callback(data); }, callback));

        // Отдельные платформы
        collections.slice(3).forEach(col => {
          u.push(callback => get(col.request, params, data => {
            data.title = col.title;
            if (['Start','Wink'].includes(col.title)) { data.small = true; data.wide = true; data.results.forEach(r => { r.promo = r.overview; r.promo_title = r.title || r.name; }); }
            if (col.title === 'ИВИ') { data.collection = true; data.line_type = 'collection'; }
            callback(data);
          }, callback));
        });

        u.push(callback => get('movie/top_rated', params, data => { data.title = Lampa.Lang.translate('title_top_movie'); data.line_type = 'top'; callback(data); }, callback));
        u.push(callback => get('tv/top_rated', params, data => { data.title = Lampa.Lang.translate('title_top_tv'); data.line_type = 'top'; callback(data); }, callback));

        // Персоны и жанры (как в оригинале)
        Lampa.Arrays.insert(u, 0, Lampa.Api.partPersons(u, 6, 'movie', u.length + 1));
        Lampa.Api.genres.movie.forEach(genre => {
          u.push(callback => get(`discover/movie?with_genres=${genre.id}`, params, data => { data.title = Lampa.Lang.translate(genre.title.replace(/[^a-z_]/g, '')); callback(data); }, callback));
        });

        return Lampa.Api.partNext(u, 6, onSuccess, onError);
      };
    }
  }
