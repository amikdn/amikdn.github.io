(function () {
  ...
  function initPlugin() {
    ...
    // Исправление дефолтной сетки (теперь гарантированно устанавливается при первой загрузке)
    if (!Lampa.Storage.has('rus_movie_view')) {
      Lampa.Storage.set('rus_movie_view', 'grid');
    }

    // Настройка вида (сетка/список)
    ...
    Lampa.SettingsApi.addParam({
      ...
      onRender: item => setTimeout(() => $('div[data-name="rus_movie_main"]').insertAfter('div[data-name="interface_size"]'), 10)
    });
        ...
        Lampa.Select.show({
          ...
          onSelect: selected => {
            ...
            if (index >= 0) {
              ...
              Lampa.Activity.push({ url: col.request, title: col.title, component: 'category_full', source: 'tmdb', page: 1 });
            }
          },
    ...
    // Логика русских новинок на главной (из второго плагина, без нарушения оригинального кода TMDB)
    if (Lampa.Storage.get('rus_movie_main', true) !== false) {
      const get = Lampa.Api.sources.tmdb.get.bind(Lampa.Api.sources.tmdb);

      Lampa.Api.sources.tmdb.main = function () {
        const periods = [{start:2023,end:2025},{start:2020,end:2022},{start:2017,end:2019},{start:2014,end:2016},{start:2011,end:2013}];
        const c1 = periods[Math.floor(Math.random() * periods.length)];
        const c2 = periods[Math.floor(Math.random() * periods.length)];
        const d1 = c1.start + "-01-01";
        const e1 = c1.end + "-12-31";
        const d2 = c2.start + "-01-01";
        const e2 = c2.end + "-12-31";
        const sorts1 = ["vote_count.desc", "vote_average.desc", "popularity.desc", "revenue.desc"];
        const sorts2 = ["vote_count.desc", "popularity.desc", "revenue.desc"];
        const sort1 = sorts1[Math.floor(Math.random() * sorts1.length)];
        const sort2 = sorts2[Math.floor(Math.random() * sorts2.length)];
        const networkIds = "2493|2859|4085|3923|3871|3827|5806|806|1191";

        const u = [];

        u.push(a => get("movie/now_playing", {}, b => { b.title = Lampa.Lang.translate("title_now_watch"); b.collection = true; b.line_type = "collection"; a(b); }, a));
        u.push(a => a({ source: "tmdb", results: Lampa.TimeTable.lately().slice(0, 20), title: Lampa.Lang.translate("title_upcoming_episodes"), nomore: true }));
        u.push(a => get("trending/all/day", {}, b => { b.title = Lampa.Lang.translate("title_trend_day"); a(b); }, a));
        u.push(a => get("trending/all/week", {}, b => { b.title = Lampa.Lang.translate("title_trend_week"); a(b); }, a));

        // Русские фильмы
        u.push(a => get(collections.find(i => i.title === 'Русские фильмы').request, {}, b => { b.title = 'Русские фильмы'; b.small = true; b.wide = true; b.results.forEach(i => { i.promo = i.overview; i.promo_title = i.title || i.name; }); a(b); }, a));
        // Русские сериалы
        u.push(a => get(collections.find(i => i.title === 'Русские сериалы').request, {}, b => { b.title = 'Русские сериалы'; a(b); }, a));
        u.push(a => get("movie/upcoming", {}, b => { b.title = Lampa.Lang.translate("title_upcoming"); a(b); }, a));
        // Русские мультфильмы
        u.push(a => get(collections.find(i => i.title === 'Русские мультфильмы').request, {}, b => { b.title = 'Русские мультфильмы'; b.collection = true; b.line_type = "collection"; a(b); }, a));
        u.push(a => get("movie/popular", {}, b => { b.title = Lampa.Lang.translate("title_popular_movie"); a(b); }, a));
        u.push(a => get("trending/tv/week", {}, b => { b.title = Lampa.Lang.translate("title_popular_tv"); a(b); }, a));

        // Подборки русских фильмов
        u.push(a => get(`discover/movie?primary_release_date.gte=${d2}&primary_release_date.lte=${e2}&vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=${sort2}`, {}, b => { b.title = 'Подборки русских фильмов'; b.line_type = "top"; a(b); }, a));
        // Подборки русских сериалов
        u.push(a => get(`discover/tv?first_air_date.gte=${d1}&first_air_date.lte=${e1}&with_networks=${networkIds}&sort_by=${sort1}`, {}, b => { b.title = 'Подборки русских сериалов'; b.line_type = "top"; a(b); }, a));

        // Платформы
        collections.filter(i => !['Русские фильмы','Русские сериалы','Русские мультфильмы'].includes(i.title)).forEach(item => {
          u.push(a => get(item.request, {}, b => {
            b.title = item.title;
            if (item.title === 'Start' || item.title === 'Wink') { b.small = true; b.wide = true; b.results.forEach(r => { r.promo = r.overview; r.promo_title = r.title || r.name; }); }
            if (item.title === 'ИВИ') { b.collection = true; b.line_type = "collection"; }
            a(b);
          }, a));
        });

        u.push(a => get("movie/top_rated", {}, b => { b.title = Lampa.Lang.translate("title_top_movie"); b.line_type = "top"; a(b); }, a));
        u.push(a => get("tv/top_rated", {}, b => { b.title = Lampa.Lang.translate("title_top_tv"); b.line_type = "top"; a(b); }, a));

        const totalCats = u.length + 1;
        Lampa.Arrays.insert(u, 0, Lampa.Api.partPersons(u, 6, "movie", totalCats));

        Lampa.Api.sources.tmdb.genres.movie.forEach(genre => {
          u.push((a, fail) => get(`discover/movie?with_genres=${genre.id}`, {}, b => { b.title = Lampa.Lang.translate(genre.title.replace(/[^a-z_]/g, "")); a(b); }, fail));
        });

        return function (build, empty) {
          Lampa.Api.partNext(u, 6, build, empty);
        };
      };
    }
  }
