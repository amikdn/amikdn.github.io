(function () {
  ...
  const PARSER_ICON = `<svg width="24" height="24" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
  ...
  const COLOR_OK = '#64e364';
  const COLOR_FAIL = '#ff2121';
  const COLOR_AUTH = '#000';

  const isAlwaysOnline = (server) => server.id === 'freebie_tom_ru' || server.id === 'Myjacket';

  const getServerUrl = (server) => (server.protocol || '') + server.baseUrl;

  const pageHttps = (typeof location !== 'undefined' && location.protocol === 'https:');
  const getRequestProtocol = (server) => server.protocol || (pageHttps ? 'https://' : 'http://');
  ...
  function checkServerStatus(server, callback) {
    ...
    const url = `${getRequestProtocol(server)}${server.baseUrl}/api/v2.0/indexers/status:healthy/results?apikey=${server.key}`;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 3000);

    fetch(url, { signal: controller.signal })
      .then((res) => { clearTimeout(timer); callback(server, res.status === 200, res.status); })
      .catch(() => { clearTimeout(timer); callback(server, false); });
  }

  function updateServerStatusInSettings() {
    setTimeout(() => {
      const firstItem = $('body > div.selectbox > div.selectbox__content.layer--height > div.selectbox__body.layer--wheight > div > div > div > div:nth-child(1) > div');
      ...
      servers.forEach((server, index) => {
        ...
        checkServerStatus(server, (srv, ok, status) => {
          if (ok) {
            ...
            const color = status === 401 ? COLOR_AUTH : COLOR_FAIL;
  ...
  function applyParserAndRefreshTorrents(item, currentActivity) {
    const server = servers.find((s) => s.id === item.url_two);
    if (server) applyServer(server);
    ...
    const act = (currentActivity && typeof currentActivity === 'object') ? currentActivity : Lampa.Storage.get('activity');
    const skipKeys = { torrents: 1, results: 1, list: 1, data: 1, items: 1, cache: 1, _cache: 1, _data: 1, state: 1, torrentList: 1, torrent_list: 1 };
    const cleanActivity = {};
    ...
    const hasActivity = Object.keys(cleanActivity).length > 0;
    ...
    const back = typeof Lampa.Activity.back === 'function' ? Lampa.Activity.back : () => window.history.back();
    back();
    setTimeout(() => {
  ...
  function closeParserSelectAndRestore(controllerName) {
    const name = controllerName || Lampa.Controller.enabled()?.name;
    if (name) {
  ...
  async function showServerSwitchMenu() {
    ...
    const controllerBeforeSelect = Lampa.Controller.enabled()?.name || '';

    const showCheckedList = (checkedServers) => {
      const backTo = controllerBeforeSelect || Lampa.Controller.enabled()?.name;
      Lampa.Select.show({
        title: 'Меню смены парсера',
        items: checkedServers.map((s) => getServerSelectItem(s)),
        onBack: () => closeParserSelectAndRestore(backTo),
        onSelect: (item) => applyParserAndRefreshTorrents(item, currentActivity),
      });
    };

    Lampa.Select.show({
      title: 'Меню смены парсера',
      items: servers.map((s) => getServerSelectItem(s)),
      onBack: () => closeParserSelectAndRestore(controllerBeforeSelect),
      onSelect: (item) => applyParserAndRefreshTorrents(item, currentActivity),
    });
  ...
  function startEmptyObserver() {
    stopEmptyObserver();
    emptyObserver = new MutationObserver(rafThrottle(() => {
  ...
  let parserButtonObserver = null;

  function startParserButtonObserver() {
    if (parserButtonObserver) return;
    parserButtonObserver = new MutationObserver(rafThrottle(() => {
  ...
  if (!Lampa.Storage.get('jack', false)) {
    ...
    const def = servers.find((s) => s.id === 'jac_red');
