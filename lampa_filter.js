(function () {
  ...
  /* Фильтр карточек по рейтингу Lampa.
     ...
     Рейтинг Lampa — это не поле в базе TMDB, а пересчёт реакций
     (🔥 нравится, 😐 задумался, 💩 и т.д.) с сервиса реакций. Значит
     отфильтровать список запросом «дай мне только 8+» невозможно: сервер
     каталога про эти оценки ничего не знает.

     Поэтому фильтр работает на стороне приложения: карточки приходят как
     обычно, плагин узнаёт их рейтинг и убирает те, что ниже порога.

     Отсюда два следствия, о которых честно сказано в описании настроек:
     1. Список становится реже — из 20 карточек при пороге 8 может остаться
        три. Lampa сама подгрузит следующую страницу, потому что скрытые
        карточки не занимают места.
     2. На каждую новую карточку нужен один запрос. Дальше он бесплатный:
        ответ лежит в кеше, в том числе в кеше плагина card_overlay, если он
        установлен. */
