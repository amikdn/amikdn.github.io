(function () {
  ...
  var stepped_aside = false;

  function ownedByPlugin() {
    return stepped_aside || !!window.nova_online_plugin;
  }

  function stepAside() {
    if (stepped_aside) return;
    stepped_aside = true;
    try {
  ...
  function enabled() { return !ownedByPlugin() && get(ENABLED_KEY, true) !== false; }
    ...
    try {
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: {
          ...
          description: ownedByPlugin()
            ? 'Отключён: установлен онлайн-плагин с этим же интерфейсом, он рисует его сам'
