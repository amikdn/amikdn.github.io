(function () {
  ...
  function hookController() {
    ...
    Lampa.Controller.add = function (name, object) {
      if (name === 'content' && object && !object.nova_hooked) {
        ...
        // the host plugin refocuses its own (now hidden) card on every toggle,
        // which lands on the first visible selector: keep our focus instead
  ...
  // the host plugin reloads through Lampa.Activity.replace() on a source switch,
  // which wipes everything we built. the snapshot below outlives that reload so
  // the header comes back untouched instead of flashing a "looking for" panel
  var carry = null;
  ...
  // while a source, season or voice switch is in flight the host plugin may
  // re-focus its own collection, which lands on the first selector (the header
  // button). the lock keeps the chip the user pressed as the focus target.
  function lockFocus(key) {
    ui_lock = key || '';
    ui_lock_time = ui_lock ? Date.now() : 0;
    if (ui_lock) ui_focus = ui_lock;
  }
  ...
  function uiFrame() {
    ...
    if (!ui.root.parent().length || !$.contains(host, ui.root[0])) {
      ...
      // the host empties its scroll body on every reload, which strips the
      // handlers off everything we keep, so wire the survivors up again
      rebind();
    }
    return ui.root;
  }
  ...
  function rebind() {
    ...
    ui.rows.find('.nova-toolbar [data-nova-focus]').each(function () {
      ...
      bind(box, function () { uiToggle(key); });
    });

    // an open drop row would keep stale handlers, the toolbar reopens it
    ui.rows.find('.nova-drop').remove();
  }

  // ----------------------------------------------------------------- carry

  function carrySave(key) {
    if (!movie || !ui.hero || !ui.hero.parent().length) {
      carry = null;
      return;
    }
    var hero = ui.hero.clone();
    var toolbar = ui.rows.find('.nova-toolbar').first().clone();
    hero.find('.focus').removeClass('focus');
    hero.removeClass('focus');
    toolbar.find('.focus').removeClass('focus');
    toolbar.find('.nova-chip--active').removeClass('nova-chip--active');

    carry = {
      id: movie.id,
      key: key || '',
      hero: hero.length ? hero[0].outerHTML : '',
      rows: toolbar.length ? toolbar[0].outerHTML : '',
      grid: !!(ui.list && ui.list.hasClass('nova__list--grid')),
      time: Date.now()
    };
  }

  function carryReady() {
    if (!carry || !carry.hero || !movie) return false;
    if (carry.id !== movie.id) return false;
    return Date.now() - carry.time < 30000;
  }

  function carryRestore() {
    ui.hero_box.empty();
    ui.rows.empty();
    ui.play = null;
    ui.next = null;
    ...
    ui.hero = $(carry.hero);
    ui.hero_box.append(ui.hero);

    // the art is already in the browser cache, so skip the fade-in
    var back = ui.hero.find('.nova-hero__bg');
    var art = back.find('img')[0];
    if (art && art.getAttribute('src')) {
      back.addClass('nova-hero__bg--loaded');
      art.onload = function () { back.addClass('nova-hero__bg--loaded'); };
      art.onerror = function () {};
    }

    var play = ui.hero.find('[data-nova-focus="hero"]').first();
    if (play.length) {
      ui.play = play;
      bindPlay(play);
    }
    var next = ui.hero.find('[data-nova-focus="hero-next"]').first();
    if (next.length) {
      ui.next = next;
      bindNext(next);
    }
    ...
    if (carry.rows) {
      var toolbar = $(carry.rows);
      ui.rows.append(toolbar);
      toolbar.find('[data-nova-focus]').each(function () {
        var box = $(this);
        var key = box.attr('data-nova-focus');
        bind(box, function () { uiToggle(key); });
      });
    ...
    if (carry.grid) ui.list.addClass('nova__list--grid');
    if (carry.key) lockFocus(carry.key);
  }
  ...
  function chooseOption(group, index) {
    if (!filter || typeof filter.onSelect !== 'function') return;
    ui_open = '';
    lockFocus(group.stype);
    buildRows();
    refreshCollection();
    focusChip(group.stype);
    carrySave(group.stype);
  ...
  function loadingPanel() {
    ...
    // the header survived a plain reload, or comes back from the snapshot after
    // the host reloaded the whole activity: either way it must not blink
    if (!(ui.hero && ui.hero.parent().length) && carryReady()) carryRestore();

    // switching source, season or voice must only refresh the list:
    // the header and the chips stay in place, exactly like the plugin does
  ...
  function draw() {
    ...
    // reuse the header we had before the host reloaded, so the poster stays put
    // and only the labels underneath it change
    if (!(ui.hero && ui.hero.parent().length) && carryReady()) carryRestore();
