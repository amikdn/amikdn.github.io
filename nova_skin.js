(function () {
  ...
  function siblingInstalled() {
    return !!window.nova_online_plugin;
  }
  ...
  function scope() {
    ...
    if (!body.find('.nova-skin-root').length &&
        (body.find('.nova__rows').length || body.find('.nova-hero').length ||
         body.find('.nova-card').length)) {
      stepAside();
      return null;
    }
  ...
  function buildRows() {
    ...
    var chip_count = toolbar.find('.nova-chip').length;
    if (chip_count > 5) toolbar.addClass('nova-toolbar--tighter');
    else if (chip_count > 4) toolbar.addClass('nova-toolbar--tight');
