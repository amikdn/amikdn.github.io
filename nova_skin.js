  ...
  function logoLoad(done) {
    ...
    try {
      url = Lampa.TMDB.api(kind + '/' + movie.id + '/images?api_key=' + Lampa.TMDB.key() +
        '&include_image_language=' + logoLang() + ',en,null');
    } catch (e) {
      url = '';
    }
    if (!url) return done('');
