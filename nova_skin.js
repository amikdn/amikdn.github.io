(function () {
  ...
  var NOVA_BUILD = "2026-08-19 13:55:12";
  ...
  function showTop(ctx) {
    var seen = null;
    [0, 60, 220].forEach(function (ms, step) {
      setTimeout(function () {
        try {
          if (!ctx.body || !ctx.body.length) return;
          var focused = ctx.body.find('.focus').first();
          var node = focused.length ? focused[0] : null;
          if (!step || !seen) seen = node;
          else if (node && node !== seen) return;
          toTop(ctx);
        } catch (e) {}
      }, ms);
    });
