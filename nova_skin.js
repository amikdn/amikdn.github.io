(function () {
  ...
  var NOVA_BUILD = "2026-08-18 22:10:12";
  ...
  function scheduleBurst() {
    clearTimeout(burst);
    var steps = [0, 60, 160, 400, 800];
    steps.forEach(function (ms) {
      burst = setTimeout(function () {
        waiting = 0;
        draw();
      }, ms);
    });
  ...
  function ours(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      try {
        if (!target.closest('.nova-hero,.nova-pills,.nova-seasons,.nova-voices,.nova-actors,.nova-info,.nova-note,.nova-foot,.nova-empty,.nova-folder')) return false;
      } catch (e) {
        return false;
      }
      mine++;
