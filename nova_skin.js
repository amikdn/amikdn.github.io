(function () {
  ...
  function pageBackground() {
    var nodes = [document.querySelector('.activity--active'), document.body];
    for (var i = 0; i < nodes.length; i++) {
      if (!nodes[i]) continue;
      var value = '';
      try { value = window.getComputedStyle(nodes[i]).backgroundColor || ''; } catch (e) { value = ''; }
      if (value && value !== 'transparent' && value.indexOf('rgba(0, 0, 0, 0)') === -1) return value;
    }
    return '#0a0b11';
  }

  function parkStart() {
    if (parked || !ui.root) return;
    try {
      var node = ui.root[0];
      if (!node || !document.body || !document.body.contains(node)) return;
      var rect = node.getBoundingClientRect();
      if (!rect.width) return;

      var frame = { left: 0, top: 0 };
      try {
        if (root && root.length) {
          var box = root[0].getBoundingClientRect();
          frame.left = Math.max(0, Math.round(box.left));
          frame.top = Math.max(0, Math.round(box.top));
        }
      } catch (e) {}

      ui.stage = $('<div class="nova-skin-stage"></div>').css({
        background: pageBackground(),
        left: frame.left + 'px',
        top: frame.top + 'px'
      });
      ui.root.addClass('nova-skin-park').css({
        left: Math.round(rect.left - frame.left) + 'px',
        top: Math.round(rect.top - frame.top) + 'px',
        width: Math.round(rect.width) + 'px',
        height: Math.max(0, Math.round(window.innerHeight - rect.top)) + 'px'
      });

      ui.stage.append(ui.root);
      $('body').append(ui.stage);
      parked = true;
      clearTimeout(park_timer);
      park_timer = setTimeout(parkDrop, 12000);
    } catch (e) {}
  }
  ...
  function stageDrop() {
    if (!ui.stage) return;
    try {
      if (ui.root) ui.root.detach();
      ui.stage.remove();
    } catch (e) {}
    ui.stage = null;
  }

  function parkRelease() {
    clearTimeout(park_timer);
    park_timer = null;
    if (!parked) return;
    parked = false;
    if (ui.root) {
      try {
        ui.root.removeClass('nova-skin-park').css({
          left: '',
          top: '',
          width: '',
          height: ''
  ...
  function parkDrop() {
    var node = parked ? ui.root : null;
    parkRelease();
    try {
      if (node && (!host || !$.contains(host, node[0]))) node.detach();
    } catch (e) {}
  }
  ...
  function chooseSource(item) {
    ...
    switchStart('source');
    parkStart();
    try { host_filter.onSelect('sort', item); } catch (e) {}
  }
  ...
  function chooseOption(group, index) {
    ...
    try { host_filter.onSelect('filter', { stype: group.stype }, { index: index }); } catch (e) {}
  }
