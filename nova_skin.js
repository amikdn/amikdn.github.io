(function () {
  ...
  var NOVA_BUILD = "2026-08-18 17:03:16";
  ...
  function openNativeFilter() {
    try {
      var ctx = context();
      var filter = (ctx && ctx.filter) || filters[0];
      if (!filter || typeof filter.show !== 'function') return false;
  ...
  function restoreFocus(ctx) {
    ...
    try {
      var list = ctx.body.find(memo.kind === 'card' ? '.online-prestige--full' : '.' + memo.kind);
      if (!list.length) return false;
      var at = Math.max(0, Math.min(memo.index, list.length - 1));
      return focusNode(list.eq(at));
    } catch (e) {}
    return false;
  }
