(function () {
  ...
  var NOVA_BUILD = "2026-08-18 16:26:31";
  ...
  function buildPills(ctx) {
    ...
    if (voice && voice.items && voice.items.length === 1) {
  ...
  function buildVoices(ctx) {
    ...
    if (!group || !group.items || group.items.length < 2) return null;
  ...
  function draw() {
    ...
    try {
      ...
      if (!ctx.body.find('.nova-hero').length) {
        ...
        tail.push(note(ctx.serial ? 'СЕРИИ' : voices ? 'ФАЙЛЫ' : 'ОЗВУЧКА'));
  ...
  function ours(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      try {
        if (!target.closest('.nova-hero,.nova-pills,.nova-seasons,.nova-voices,.nova-actors,.nova-info,.nova-note')) return false;
      } catch (e) {
        return false;
      }
      mine++;
