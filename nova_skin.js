(function () {
  ...
  function playableNode(node) {
    try {
      ...
      var box = $._data(node, 'events');
      if (!box) return false;
      return !!box['hover:enter'];
    } catch (e) {
      return true;
    }
  }
  ...
  function readCard(node, index) {
    ...
    return {
      origin: origin,
      index: index,
      soon: !origin.hasClass('online-prestige--folder') && !playableNode(node),
  ...
  function pickResume(full) {
    var i;
    var list = (full || []).filter(function (entry) { return !entry.soon; });
  ...
  function nextItem(target) {
    ...
    for (var i = target.index + 1; i < items.length; i++) {
      if (!items[i].soon) return items[i];
    }
    return null;
  }
  ...
  function buildCard(item, compact) {
    ...
    if (item.soon) {
      card.removeClass('selector').addClass('nova-card--soon');
      card.removeAttr('data-nova-focus');
    } else {
