(function() {
  ...
  function isOnline() {
    try {
      ...
      if (!a || !a.activity) return false;
      var root = a.activity.render();
      if (!root || !root.length) return false;
      if (!root.hasClass('explorer')) root = root.find('.explorer').first();
      return !!root.find('.online-prestige--full,.online-prestige--folder,.torrent-list').length;
    } catch (e) {}
    return false;
  }
...
`;
  ...
  function draw() {
    ...
    var skinRoot = ctx.body.find('.z01');
    if (!skinRoot.length) {
      skinRoot = $('<div class="z01"></div>');

      var hero = buildHero(ctx);
      if (hero) skinRoot.append(hero);

      var toolbar = buildToolbar(ctx);
      if (toolbar) skinRoot.append(toolbar);

      ctx.body.prepend(skinRoot);
    }
