(function() {
  ...
  // Объект с альтернативными URL для отдельных балансеров
  var alternateURLs = {
    filmix: "http://rc.bwa.to/rc/fxapi",
    fxapi: "http://rc.bwa.to/rc/fxapi",
    kinopub: "http://vcdn3.skaz.tv/lite/kinopub"
    // Добавьте другие балансеры по необходимости
  };
  ...
  function component(object) {
    ...
    var last;
    var source;
    // Получаем выбранный балансер (из Storage или по умолчанию)
    var balanser = (Lampa.Storage.get('online_balanser', 'filmix')).toLowerCase();
    var initialized;
    var balanser_timer;
    var images = [];
    ...
    // =========================================================================
    // Формирование URL запроса с учётом альтернативных адресов
    this.requestParams = function(url) {
      if (balanser && alternateURLs[balanser.toLowerCase()]) {
        url = alternateURLs[balanser.toLowerCase()];
      } else {
        url = Defined.localhost + "lite/" + balanser;
      }
      var query = [];
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        ...
        if (filter_sources.length) {
          ...
          if (lowPriorityBalancers.indexOf(balanser) !== -1 && filter_sources.some(function(item) { return lowPriorityBalancers.indexOf(item) === -1; })) {
            balanser = filter_sources.find(function(item) { return lowPriorityBalancers.indexOf(item) === -1; });
          }
    ...
    // Новая логика обработки ответа от сервера
    this.parse = function(str) {
      var json = Lampa.Arrays.decodeJson(str, {});
      // Если сервер возвращает объект RCH – проверяем содержимое поля result
      if (json.rch) {
        // Если result уже массив источников – отобразим его
        if (json.result && Array.isArray(json.result)) {
          this.displayOptions(json.result);
          return;
        }
        // Если result строка – разделим её по переносам и создадим массив
        if (typeof json.result === 'string') {
          var options = json.result.split('\n').filter(function(item) { return item.trim().length > 0; });
          this.displayOptions(options);
          return;
        }
        // Если формат не соответствует, используем стандартную обработку RCH
    ...
    // Новая функция для отображения списка источников (опций)
    this.displayOptions = function(options) {
      console.log("Получен список источников:", options);
      var html = $('<div class="options-list"></div>');
      options.forEach(function(opt) {
        // Можно доработать отображение: разбить строку на название и качества и т.д.
        html.append('<div class="option-item">' + opt + '</div>');
      });
      scroll.clear();
      scroll.append(html);
      Lampa.Controller.enable('content');
    ...
    this.similars = function(json) {
      ...
      json.forEach(function(elem) {
        ...
        item.on('hover:enter', function() { _this6.reset(); _this6.request(elem.url); })
  ...
  function startPlugin() {
    ...
    Lampa.Template.add('lampac_css', "\n        <style>\n        @charset 'UTF-8';.online-prestige{position:relative;-webkit-border-radius:.3em;border-radius:.3em;background-color:rgba(0,0,0,0.3);display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex}/* ... остальной CSS ... */\n        </style>\n    ");
