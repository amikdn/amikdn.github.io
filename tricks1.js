...
(function() {
  ...
  var Defined = {
    ...
    apn: '',
    fxapi: {
      token: '5c8dc18ea0cd702ac1338ff9aa321d55',
      unic_id: 'waoqeEEMtP8skyG4',
      proxy_url: 'http://cors.cfhttp.top/',
  ...
  function component(object) {
    ...
    var balansers_with_search = ['kinotochka', 'kinopub', 'lumex', 'filmix', 'filmixtv', 'fxapi', 'redheadsound', 'animevost', 'animego', 'animedia', 'animebesst', 'anilibria', 'rezka', 'rhsprem', 'kodik', 'remux', 'animelib', 'kinoukr', 'rc/filmix', 'rc/fxapi', 'rc/kinopub', 'rc/rhs', 'vcdn'];
    ...
    this.requestParams = function(url) {
      if (balanser && balanser.toLowerCase() === 'fxapi') {
        var query = [];
        query.push('story=' + encodeURIComponent(object.clarification ? object.search : object.movie.title || object.movie.name));
        query.push(Defined.fxapi.dev_token);
        url = Defined.fxapi.proxy_url + Defined.fxapi.api_url + 'search?' + query.join('&');
      } else {
        var query = [];
    ...
    this.parseFilmix = function(json) {
      var _this = this;
      if (json && json.length) {
        var cards = json.filter(function(c) {
          ...
          return c.year > movieYear - 2 && c.year < movieYear + 2;
        });
        var card = cards.find(function(c) { return c.year == movieYear; }) || cards[0];

        if (card) {
          var url = Defined.fxapi.proxy_url + Defined.fxapi.api_url + 'post/' + card.id + '?' + Defined.fxapi.dev_token;
          network["native"](url, function(found) {
            if (found && Object.keys(found).length) {
              _this.displayFilmix(found);
            } else {
              _this.doesNotAnswer();
            }
          }, function() {
    ...
    this.displayFilmix = function(data) {
      var _this = this;
      var videos = [];
      ...
      if (pl_links.playlist && Object.keys(pl_links.playlist).length > 0) {
        for (var season in pl_links.playlist) {
          var episodes = pl_links.playlist[season];
          var transl_id = 0;
          for (var voice in episodes) {
            transl_id++;
            var episode_voice = episodes[voice];
            for (var id in episode_voice) {
              var file = episode_voice[id];
              var quality_eps = file.qualities.filter(function(q) { return q <= Defined.fxapi.max_quality; });
              var max_quality = Math.max.apply(null, quality_eps);
              var stream_url = file.link.replace('%s.mp4', max_quality + '.mp4');
              var s_e = stream_url.match(/s(\d+)e(\d+?)_\d+\.mp4/i);
              if (s_e) {
                videos.push({
                  title: s_e[2] + ' ' + Lampa.Lang.translate('torrent_serial_episode'),
                  url: stream_url,
                  quality: max_quality + 'p',
                  qualitys: quality_eps.reduce((acc, q) => { acc[q + 'p'] = stream_url.replace(max_quality + '.mp4', q + '.mp4'); return acc; }, {}),
                  episode: parseInt(s_e[2]),
                  season: parseInt(s_e[1]),
                  voice_name: voice,
                  method: 'play'
                });
              }
        ...
        for (var id in pl_links.movie) {
          ...
          var quality_eps = file.link.match(/.+\[(.+[\d])[,]+?\].+/i)[1].split(',').filter(q => q <= Defined.fxapi.max_quality);
          var max_quality = Math.max.apply(null, quality_eps);
          var file_url = file.link.replace(/\[(.+[\d])[,]+?\]/i, max_quality);
          videos.push({
            title: file.translation,
            url: file_url,
            quality: max_quality + 'p',
            qualitys: quality_eps.reduce((acc, q) => { acc[q + 'p'] = file_url.replace(max_quality, q); return acc; }, {}),
            voice_name: file.translation,
            method: 'play'
          });
        }
      }
  ...
  function startPlugin() {
    ...
    Defined.fxapi.dev_token = 'user_dev_apk=2.0.1&user_dev_id=' + Defined.fxapi.unic_id + '&user_dev_name=Lampa&user_dev_os=11&user_dev_vendor=FXAPI&user_dev_token=' + Defined.fxapi.token;
    var manifst = {
      type: 'video',
      version: '2',
      name: '4m1K',
