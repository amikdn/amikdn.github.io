(function() {
  ...
  // Функция account – подставляет параметры из хранилища (без фиксированной подмены в данном случае)
  function account(url) {
    ...
    if (url.indexOf('token=') === -1) {
      var token = ''; // Если нужен фиксированный токен – можно подставить здесь
  ...
  function component(object) {
    ...
    // Подмена URL: если выбран балансер "filmixtv", заменяем URL на фиксированный
      ...
      return new Promise(function(resolve, reject) {
        ...
        var fin = function fin(call) {
          ...
          network.silent(account(url), function(json) {
            ...
            filter_sources = [];
            sources = {};
            // Пример фильтрации – можно изменить при необходимости
    ...
    this.parse = function(str) {
      ...
      try {
        ...
        if (items.length === 1 && items[0].method === 'link' && !items[0].similar) {
          ...
          _this.replaceChoice({ season: 0 });
          _this.request(items[0].url);
        } else {
          ...
          if (videos.length) {
            if (buttons.length) {
              ...
              var select_voice_url = _this.getChoice(balanser).voice_url;
              var select_voice_name = _this.getChoice(balanser).voice_name;
              ...
              if (find_voice_url && !find_voice_url.active) {
                _this.replaceChoice({ voice: buttons.indexOf(find_voice_url), voice_name: find_voice_url.text });
                _this.request(find_voice_url.url);
              } else if (find_voice_name && !find_voice_name.active) {
                _this.replaceChoice({ voice: buttons.indexOf(find_voice_name), voice_name: find_voice_name.text });
                _this.request(find_voice_name.url);
              } else {
                if (find_voice_active) {
                  _this.replaceChoice({ voice: buttons.indexOf(find_voice_active), voice_name: find_voice_active.text });
                }
            ...
            if (similar.length) {
              _this.similars(similar);
              _this.activity.loader(false);
            } else {
              filter_find.season = items.map(function(s) { return { title: s.text, url: s.url }; });
              var select_season = _this.getChoice(balanser).season;
              var season = filter_find.season[select_season];
              if (!season) season = filter_find.season[0];
              _this.request(season.url);
            }
          } else {
            _this.doesNotAnswer(json);
          }
        }
      } catch (e) {
    ...
    this.draw = function(items) {
      ...
      scroll.clear();
      scroll.append(Lampa.Template.get('lampac_prestige_watched', {}, true));
      this.updateWatched();
  ...
  function startPlugin() {
    ...
    // Регистрируем плагин – если Manifest.plugins должен быть объектом, заменяем его
    ...
    Lampa.Template.add('lampac_css', "\n        <style>\n        @charset 'UTF-8';.online-prestige{position:relative;-webkit-border-radius:.3em;border-radius:.3em;background-color:rgba(0,0,0,0.3);display:-webkit-box;display:-webkit-flex;display:-moz-box;display:-ms-flexbox;display:flex}\n        .online-prestige__body{padding:1.2em;line-height:1.3;flex-grow:1;position:relative}\n        /* Дополнительные стили... */\n        </style>\n    ");
    $('body').append(Lampa.Template.get('lampac_css', {}, true));
    startPlugin = null;
  }

  // Функция resetTemplates – регистрирует шаблоны HTML
