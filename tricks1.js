...
// ======================================================================= // Функция account() с фиксированными uid и token function account(url) { url = url + ''; // Если параметр account_email отсутствует – добавляем его из хранилища, если имеется if (url.indexOf('account_email=') === -1) { var email = Lampa.Storage.get('account_email'); if (email) { url = Lampa.Utils.addUrlComponent(url, 'account_email=' + encodeURIComponent(email)); } } // Подставляем фиксированный uid var fixedUid = "qdiicjlp"; url = Lampa.Utils.addUrlComponent(url, 'uid=' + encodeURIComponent(fixedUid));

javascript
Копировать
// Подставляем фиксированный token
