(function () {
  ...
  /**
   * ============================
   * Часть 1. Добавляем кнопку "Кинопоиск" в меню Lampa
   * ============================
   */
  Lampa.Platform.tv();
  ...
  function addMenuButton(newItemAttr, newItemText, iconHTML, onEnterHandler) {
    ...
    if (window.appready) {
      ...
      // Когда Lampa "готова", вставляем кнопку
      ...
      <g fill="none" fill-rule="evenodd">
        <g fill="currentColor" fill-rule="nonzero">
          <path fill-rule="evenodd" d="
            M20,4 H172 A16,16 0 0 1 188,20 V172 A16,16 0 0 1 172,188 H20 A16,16 0 0 1 4,172 V20 A16,16 0 0 1 20,4 Z
            M20,18 H172 A2,2 0 0 1 174,20 V172 A2,2 0 0 1 172,174 H20 A2,2 0 0 1 18,172 V20 A2,2 0 0 1 20,18 Z
          "/>
  ...
  // ============================
  // Часть 2. Создаём компонент "kp_categories" — 
  //           страницу с иконками категорий вместо Select.show
  // ============================
  (function createKpCategoriesComponent() {
    /**
     * Здесь задаём список категорий (название, url, svg-иконку).
     * Можно дополнить/изменять как угодно.
     */
    var categories = [
      {
    ...
    /**
     * Компонент "kp_categories"
     * - создаёт Scroll + карточки (в стиле Lampa)
     * - при клике открывает Activity с нужным url
     */
    function KpCategories() {
      var scroll = new Lampa.Scroll({ mask: true, over: true });
      var html = Lampa.Template.js('list'); 
      // Или можно вручную: let html = $('<div class="kp-cats"><div class="kp-cats__body"></div></div>')...
      // Но проще использовать типовой template "list"
      ...
      this.create = function() {
        ...
        categories.forEach(cat => {
          ...
          item.querySelector('.card__img').style.background = 'none'; // убираем фон
          item.querySelector('.card__title').textContent = cat.title;
          item.querySelector('.card__view').remove(); // убираем "view"

          // Событие
          item.addEventListener('hover:enter', ()=>{
            // Переходим в список фильмов
            Lampa.Activity.push({
              url: cat.url,
              title: cat.title,
      ...
      this.start = function(){
        Lampa.Controller.add('content', {toggle: ()=>{ Lampa.Controller.collectionSet(this.render())}, back: this.back});
  ...
  /**
   * Теперь меняем поведение нажатия кнопки "Кинопоиск":
   * Вместо Select.show(...) открываем страницу "kp_categories"
   */
  ...
  // Перехватываем добавление кнопки: 
  // Вместо прежнего Select.show, 
  // даём onEnterHandler = onKpButtonEnter
  (function fixKpButton(){
    // Ищем кнопку, которую мы добавили выше: [data-action="kp"]
    // Если успел добавиться — переназначим ему коллбэк
    // Если не успел — слушаем app ready

    function setNewHandler() {
      let btn = Lampa.Menu.render().find('[data-action="kp"]');
      if(btn.length){
        btn.off('hover:enter'); // снимаем прежний
        btn.on('hover:enter', onKpButtonEnter);
      }
    }
  ...
  /**
   * =============================
   * Часть 3. Исходный плагин KP_API (объект KP_PLUGIN) - уже включён ниже
   * + Регистрация в Lampa (источник "KP")
   * =============================
   * (всё, что ниже — взято из вашего кода "кп финал урааааа.txt", без изменений)
   */
  if (!window.KP_PLUGIN) {
    window.KP_PLUGIN = (function () {
      ...
      function convertElem(elem) {
        ...
        if (elem.seasons_obj) {
          ...
          result.seasons = _seasons.map(function (season) {
      ...
      function kpCleanTitle(str) {
        function cleanTitle(st) {
          return st.replace(/[\s.,:;’'`!?]+/g, ' ').trim();
        }
        ...
        if (film) {
          ...
          get(url, function (film) {
            if (film.kinopoiskId) {
              ...
              function getCompliteIf(condition, method, done) {
                if (condition) get(method, done, function(){done(null);});
                else setTimeout(function(){done(null);},10);
              }
              getCompliteIf(type === 'tv', 'api/v2.2/films/' + id + '/seasons', function (seasons) {
                film.seasons_obj = seasons;
                get('api/v2.2/films/' + id + '/distributions', function (distributions) {
                  film.distributions_obj = distributions;
                  get('/api/v1/staff?filmId=' + id, function (staff) {
                    film.staff_obj = staff;
                    get('api/v2.1/films/' + id + '/sequels_and_prequels', function (sequels) {
                      film.sequels_obj = sequels;
                      get('api/v2.2/films/' + id + '/similars', function (similars) {
      ...
      function category(params, oncomplite, onerror) {
        ...
        books = books.filter(function(elem){
          return ['KP','tmdb','cub'].indexOf(elem.source)!==-1;
        });
  ...
  (function registerKPSource(){
    ...
    function addPlugin(){
      ...
      Object.defineProperty(Lampa.Api.sources,'KP',{get:function(){return KP_PLUGIN;}});
      var sources;
