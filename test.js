...
(function () {
  ...
  var NOVA_BUILD = "2026-08-18 16:26:30";
  ...
  var api = {
    ...
    up: function () {
      try {
        ...
        if (focused.closest('.nova-hero').length) {
    ...
    head: function (opt) {
      try {
        ...
        if (voices) {
          opt.scroll.append(note('ОЗВУЧКА'));
          opt.scroll.append(voices);
        }

        opt.scroll.append(note(opt.serial ? 'СЕРИИ' : voices ? 'ФАЙЛЫ' : 'ОЗВУЧКА'));
    ...
    decorateCard: function (html, element, episode, serial) {
      try {
        if (!serial) {
          ...
          if (picked && html.find('.online-prestige__title').text().trim() === ('' + picked).trim()) {
            html.addClass('nova-is-sel');
          }
          return;
        }
