(function() {
  ...
  function goExport(path) {
    ...
    var uri = account('http://185.87.48.42:2627/storage/set?path='+path+'&pathfile='+Lampa.Storage.get('lampac_profile_id', ''));
  ...
  function goImport(path, call) {
    ...
    network.silent(account('http://185.87.48.42:2627/storage/get?path='+path+'&pathfile='+Lampa.Storage.get('lampac_profile_id', '')), function(j) {
  ...
  if (!window.lwsEvent) {
    Lampa.Utils.putScript([account("http://185.87.48.42:2627/invc-ws.js")], function() {}, false, function() {
      sync();
    }, true);
  } else sync();
