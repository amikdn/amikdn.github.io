  ...
  function initPlugin() {
    ...
    function RusMainSource(source) {
      ...
      this.main = function() {
        ...
        var q = { get: Lampa.Api.sources.tmdb.get.bind(Lampa.Api.sources.tmdb) };
    ...
    if (Lampa.Storage.get('rus_movie_main') !== false) {
      ...
      for (var sourceName in Lampa.Api.sources) {
        if (Lampa.Api.sources.hasOwnProperty(sourceName)) {
          Lampa.Api.sources[sourceName].main = newSource.main;
        }
      }
      Lampa.Api.sources.tmdb.network = newSource.network;
      activateRusMain();
    }
