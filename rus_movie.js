(function () {
    ...
    // =================================================
    // 8. ГАРАНТИРОВАННОЕ ДОБАВЛЕНИЕ В МЕНЮ (через MutationObserver)
    // =================================================
    function addMenuItemForce() {
        const observer = new MutationObserver((mutations, obs) => {
            ...
            if ($list.length > 0) {
                ...
                console.log('Пункт "Русское" добавлен в меню (через MutationObserver)');
                obs.disconnect(); // Останавливаем наблюдение после успеха
            }
        });
    ...
    const Plugin = function (api) {
        ...
        this.build = function () {
            ...
            const requests = [
                function (cb) { self.get('trending/all/week', params, data => { data.title = Lampa.Lang.translate('title_main'); data.small = true; data.line_type = 'small'; cb(data); }, cb); },
                function (cb) { cb({ source: 'tmdb', results: Lampa.TimeTable.get().slice(0, 20), title: Lampa.Lang.translate('title_upcoming_episodes'), nomore: true, cardClass: (item, episode) => new CardRenderer(item, episode) }); },
                function (cb) { self.get('trending/all/day', params, data => { data.title = Lampa.Lang.translate('title_trend_day'); cb(data); }, cb); },
                function (cb) { self.get('trending/all/week', params, data => { data.title = Lampa.Lang.translate('title_trend_week'); cb(data); }, cb); },
                function (cb) { self.get(`discover/movie?vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=primary_release_date.desc&primary_release_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Русские новинки'); data.small = true; data.wide = true; data.results.forEach(i => { i.promo = i.overview; i.promo_title = i.title || i.name; }); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_original_language=ru&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Русские сериалы'); cb(data); }, cb); },
                function (cb) { self.get('movie/upcoming', params, data => { data.title = Lampa.Lang.translate('title_upcoming'); cb(data); }, cb); },
                function (cb) { self.get(`discover/movie?vote_average.gte=5&vote_average.lte=9.5&with_genres=16&with_original_language=ru&sort_by=primary_release_date.desc&primary_release_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Русские мультфильмы'); data.small = true; data.line_type = 'small'; cb(data); }, cb); },
                function (cb) { self.get('movie/popular', params, data => { data.title = Lampa.Lang.translate('title_popular_movie'); cb(data); }, cb); },
                function (cb) { self.get('movie/top_rated', params, data => { data.title = Lampa.Lang.translate('title_top_movie'); data.line_type = 'top'; cb(data); }, cb); },
                function (cb) { self.get(`discover/movie?primary_release_date.gte=${fromYear}&primary_release_date.lte=${toYear}&vote_average.gte=5&vote_average.lte=9.5&with_original_language=ru&sort_by=${tvSortBy}`, params, data => { data.title = Lampa.Lang.translate('Подборки русских фильмов'); data.line_type = 'top'; cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?first_air_date.gte=${fromYear2}&first_air_date.lte=${toYear2}&with_networks=2493|2859|4085|3923|3871|3827|5806|806|1191&sort_by=${sortBy}`, params, data => { data.title = Lampa.Lang.translate('Подборки русских сериалов'); data.line_type = 'wide'; cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=2493&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('СТС'); data.wide = true; data.small = true; data.results.forEach(i => { i.promo = i.overview; i.promo_title = i.title || i.name; }); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=2859&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('ТНТ'); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=3827&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('KION'); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=3923&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Premier'); data.collection = true; data.line_type = 'small'; cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=5806&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('OKKO'); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=3923&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Premier'); data.wide = true; data.small = true; data.results.forEach(i => { i.promo = i.overview; i.promo_title = i.title || i.name; }); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=806&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Wink'); cb(data); }, cb); },
                function (cb) { self.get(`discover/tv?with_networks=1191&sort_by=first_air_date.desc&air_date.lte=${today}`, params, data => { data.title = Lampa.Lang.translate('Start'); cb(data); }, cb); },
                function (cb) { self.get('movie/top_rated', params, data => { data.title = Lampa.Lang.translate('title_top_movie'); data.line_type = 'top'; cb(data); }, cb); },
                function (cb) { self.get('tv/top_rated', params, data => { data.title = Lampa.Lang.translate('title_top_tv'); data.line_type = 'top'; cb(data); }, cb); }
            ];
    ...
    const CardRenderer = function (data, episodeData) {
        ...
        this.build = function () {
            ...
            if (item.release_year === '0000') hideCard(this.card.find('.card__age'));
            else this.card.querySelector('.card__age').innerText = item.release_year;
        ...
        this.render = function () { this.image(); };
    ...
    // =================================================
    // 12. Инициализация (через appready + fallback)
    // =================================================
    function initPlugin() {
        // Добавляем пункт в меню через MutationObserver
        addMenuItemForce();
    ...
    // Fallback: если appready не сработал — запускаем через 5 сек
