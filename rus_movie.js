(function () {
  ...
  function initPlugin() {
    ...
    function injectRusModalStyle() {
      if (window.__rusmovies_modal_style__) return;
      window.__rusmovies_modal_style__ = true;
      var css =
        ".rusmodal{display:flex;flex-direction:column;gap:.5em;padding-right:1em}\n" +
        ".rusmodal__section{font-size:.85em;opacity:.55;text-align:center;padding:.2em 0}\n" +
        ".rusmodal__divider{border-top:1px solid rgba(255,255,255,.08);margin-top:.2em}\n" +
        ".rusmodal__item{display:flex;align-items:center;gap:.8em;padding:.7em 1em .7em 1em;border-radius:.7em;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);cursor:pointer;user-select:none;box-sizing:border-box}\n" +
        ".rusmodal__item.focus{border-color:#fff!important;background:rgba(255,255,255,.1)}\n" +
        ".rusmodal__icon{width:2.2em;height:1.7em;flex-shrink:0;display:flex;align-items:center;justify-content:center}\n" +
        ".rusmodal__icon svg{width:100%;height:100%}\n" +
        ".rusmodal__label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:1.1em}\n" +
        ".rusmodal__action{display:block;text-align:center;padding:.6em 1em;border-radius:.7em;cursor:pointer;user-select:none;font-size:1em;box-sizing:border-box}\n" +
        ".rusmodal__action--close{background:rgba(66,133,244,.55);border:1px solid rgba(255,255,255,.15)}\n" +
        ".rusmodal__action--close.focus{border-color:#fff!important}\n";
    ...
    function openRusModal() {
      ...
      setTimeout(function () {
        ...
        var modal = $('<div class="rusmodal"></div>');
        ...
        modal.append($('<div class="rusmodal__section">Фильмы и сериалы</div>'));

        var movieCollections = collections.filter(function(item) {
          return item.title === 'Русские фильмы' || item.title === 'Русские сериалы' || item.title === 'Русские мультфильмы';
        });

        movieCollections.forEach(function(col) {
          var row = $('<div class="rusmodal__item selector" tabindex="0"></div>');
          ...
          row.on('click', function (e) { if (e && e.preventDefault) e.preventDefault(); if (e && e.stopPropagation) e.stopPropagation(); blurAfterMouse(e); });
          modal.append(row);
        });

        modal.append($('<div class="rusmodal__divider"></div>'));
        modal.append($('<div class="rusmodal__section">Платформы и каналы</div>'));

        var networkCollections = collections.filter(function(item) {
          return item.title !== 'Русские фильмы' && item.title !== 'Русские сериалы' && item.title !== 'Русские мультфильмы';
        });

        networkCollections.forEach(function(col) {
          var row = $('<div class="rusmodal__item selector" tabindex="0"></div>');
          row.append($('<div class="rusmodal__icon"></div>').append($(col.svg)));
          row.append($('<div class="rusmodal__label"></div>').text(col.title));
          row.on('hover:enter', function () {
        ...
        if (typeof Lampa.Modal !== 'undefined' && Lampa.Modal.open) {
          Lampa.Modal.open({ title: 'Русское', html: modal, size: 'medium', scroll_to_center: true, onBack: function () { closeModal(); } });
        }
      }, 200);
    }
